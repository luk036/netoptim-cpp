...
namespace netoptim_detail {
    ...
    /** @brief Adapter: evaluates the scaling-ratio constraint for one edge.
     *
     * Provides the eval / grad interface expected by NetworkOracle, adapting
     * a cost function (edge -> (a_ij, a_ji)) to the constraint
     * h_ij(x) = min(pi - a_ji, a_ij - psi) with x = (pi, psi) in log scale.
     *
     ...
     */
    template <typename Graph, typename Fn> class Ratio {
      public:
...
class OptScalingOracle {
    ...
    using Ratio = netoptim_detail::Ratio<Graph, Fn>;
