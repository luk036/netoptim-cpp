...
public:
    ...
    vector<int> find_neg_cycle(unordered_map<int, R>& dist, function<R(pair<int, int>)> get_weight) {
        int n = graph_.size();
        vector<int> prev(n, -1);
        vector<int> cycle;
        int last = -1;
...
R max_parametric(
    ...
    Cycle<R, V> cycle;
    while (true) {
        for (auto& ci : ncf.find_neg_cycle(dist, [&](pair<V, V> edge) { return omega.distance(ratio, edge); })) {
